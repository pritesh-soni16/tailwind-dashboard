...
export const useCustomeBreakpoints = (customBreakpoints: Breakpoints) => {
  ...
  const handleResize = () => {
    ...
    const { mobile, tablet, laptop, m_laptop, xl_desktop, xxl_desktop } = {
      ...
      xl_desktop: customBreakpoints?.xl_desktop ?? DEFAULT_BREAKPOINTS.xl_desktop,
      xxl_desktop: customBreakpoints?.xxl_desktop ?? DEFAULT_BREAKPOINTS.xxl_desktop,
    };
  ...
  return { isSMobile, isMobile, isTablet, isLaptop, isMLaptop, isXLDesktop, isXXLDesktop, isLessThan1199 };
};